...
check('versionCode 2228','versionCode 2228' in gradle)
check('versionName 2.2.28',"versionName '2.2.28'" in gradle)
check('applicationId preserved',"applicationId 'com.grassrootsclubhub.universal'" in gradle)
check('native marker version','GrassrootsClubHub/2.2.28' in main)
...
check('About version 2.2.28','App version 2.2.28' in index)
check('manifest name PitchKind',manifest.get('name')=='PitchKind' and manifest.get('short_name')=='PitchKind')
check('manifest version 2.2.28',manifest.get('version')=='2.2.28')
...
check(
    'v2.2.28 native User-Agent constant',
    'private static final String NATIVE_USER_AGENT' in main
    and 'GrassrootsClubHub/2.2.28' in main
...
check(
    'v2.2.28 workflow and artifact gate',
    'Android APK v2.2.28 PitchKind' in apk_workflow
    and 'verify_v2_2_28_pitchkind_ui.py' in apk_workflow
    and 'App version 2.2.28' in apk_workflow
    and 'PitchKind-v2.2.28-debug.apk' in apk_workflow
    and 'GrassrootsClubHub/2.2.28' in apk_workflow,
)
...
check(
    ...
    and "renderFixtureOverview('next-match',d)" in app
    and "renderFixtureOverview('matches-next',d)" in app,
)
check(
...
check(
    ...
    and "const away=shirtColoursOnly(stored.away||pilot.away||'TBC')" in app,
)
check(
...
check(
    ...
    and "colours:shirtColoursOnly(d.club_colours||cached.colours||'TBC')" in app
...
check('compact home next-match card only exposes teams and date',
    'id="home-next-match"' in index
    and 'id="next-match-home-teams"' in index
    and 'id="next-match-home-date"' in index
    and index.find('id="next-match-card"') < index.find('id="next-fixture-dialog"'))
check('full next-match detail moved to popup',
    'id="next-fixture-dialog"' in index
    and 'id="next-match-ground"' in index
    and 'id="next-match-map-preview"' in index
    and 'function openNextFixtureDetails()' in app)
check('verified badge preferred for compact match identity',
    'function verifiedTeamBadgeUrl' in app
    and 'function teamIdentityVisualHtml' in app
    and 'shooters-hill-logo.png' in app)
check('fixture confirmation override is private team state',
    'fixtureOverrides: {}' in app
    and 'fixtureOverrides: data.selkent?.fixtureOverrides' in app
    and 'function saveFixtureConfirmation()' in app
    and "fixture_details_confirmed" in app)
check('public feed refresh cannot overwrite confirmed kickoff venue',
    'function resolvedFixture(f={})' in app
    and 'confirmedAt' in app
    and 'const grounds=Array.isArray(detail.groundOptions)?detail.groundOptions:[];if(grounds.length===1)' in app)
check('Selkent multiple grounds are parsed for coach selection',
    'function parseSelkentClubGroundOptions' in app
    and 'groundOptions:apiGrounds' in app
    and 'id="next-match-ground-select"' in index
    and 'Other / manual venue' in app)
check('Valiants home and away shirts are available and goalkeeper is not a fixture choice',
    "{home:'Green and white',away:'Blue and white'}" in app
    and 'id="next-match-kit-choice"' in index
    and '>Home</option><option value="away">Away</option>' in index
    and 'goalkeeper' not in index[index.find('id="next-fixture-dialog"'):index.find('id="match-detail-dialog"')])
check('Valiants shirt icons reflect current green and blue reference kits',
    'function valiantsKitIconHtml' in app
    and 'function teamKitIconHtml' in app
    and 'Green and white' in app
    and 'Blue and white' in app)
check('Share match image contains no player data or score fields',
    'function shareNextMatchImage()' in app
    and 'sharePngDataUrl' in app
    and 'Shared from PitchKind' in app
...
check('native PNG sharing uses read-only provider',
    ...
    and 'android:authorities="${applicationId}.share"' in txt('app/src/main/AndroidManifest.xml'))
check('v2.2.25 compact fixture styles present',
    'v2.2.25 — compact dashboard next match and confirmed fixture details' in styles
    and '.next-match-summary-card' in styles
...
check('match layouts use prominent badges plus secondary shirt visuals',
    ...
    and 'match-kit-secondary' in app
    and "matchTeamSideHtml('Home',ctx.homeTeam,ctx.homeKit)" in app
    and "matchTeamSideHtml('Away',ctx.awayTeam,ctx.awayKit)" in app
    and 'match-team-badge-slot' in styles
    and '.match-kit-secondary .match-team-kit' in styles)
...
check('confirmation and share actions are explicit and visible to staff',
    ...
    and 'id="next-match-share">Share match' in index
    and 'id="matches-next-share">Share match' in index
    and 'id="next-match-share" data-requires-edit' not in index
    and 'share.disabled=false' in app)
check('match image is badge-first and shareable before or after confirmation',
    'const confirmed=fixtureDetailsConfirmed(f)' in app
    and 'Kick-off awaiting confirmation' in app
    and 'Venue awaiting confirmation' in app
    and 'function drawShareClubIdentity' in app
    and 'function drawSharePlaceholderBadge' in app
    and 'await Promise.all([drawShareClubIdentity' in app
    and "drawShareJersey(g,270,505,ctx.homeKit,.52)" in app)
...
check('v2.2.27 share controls are visible in popup and Matches next-fixture card',
    'id="next-match-share">Share match' in index
    and 'id="matches-next-share">Share match' in index
    and "document.getElementById('matches-next-share')?.addEventListener('click',shareNextMatchImage)" in app)
check('v2.2.27 share no longer depends on confirmed fixture details',
    'Confirm fixture details before generating the match image' not in app
    and "share.title=confirmed?'Share the confirmed match information image':'Share the fixture with awaiting-confirmation labels'" in app)
...
check('v2.2.28 confirm and share live outside the scroll region in persistent footer',
    ...
    and 'id="next-match-share">Share match' in index[index.find('id="next-fixture-action-bar"'):]
