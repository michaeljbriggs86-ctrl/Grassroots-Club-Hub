...
- Repeated directory syncs must update the existing logical team rather than insert another row.
- A division change must not create a new team record.
- Duplicate cleanup must preserve the canonical/oldest team record and any linked state, profiles, invitations, attendance, notes, safeguarding data, or audit history.
- A database uniqueness guard should enforce the logical identity for active teams.
...
## 11.7 Duplicate internal club-team records

Confirmed on 2026-09-19 for Shooters Hill: repeated directory syncs created multiple active rows for the same age-group/team name.

Required fix:

- preserve the original/canonical team row
- move or retain all linked data safely
- remove empty duplicate rows
- change `sync_team_directory` to use the stable logical identity from Section 4.4
- add a database uniqueness guard so the fault cannot recur
