...
const STARTER_DATA = {
  ...
  selkent: {
    ...
    fixtureTracking: {}, fixtureAcknowledgement: {}, kitColours: {}, kitProfiles: {}, fixtureKitSelections: {}, fixtureOverrides: {}, directoryDetails: {},
...
function normalizeState(data={}){
  ...
  return {
    ...
    selkent: {
      ...
      fixtureKitSelections: data.selkent?.fixtureKitSelections && typeof data.selkent.fixtureKitSelections==='object' ? data.selkent.fixtureKitSelections : {},
      fixtureOverrides: data.selkent?.fixtureOverrides && typeof data.selkent.fixtureOverrides==='object' ? data.selkent.fixtureOverrides : {},
...
function decodeSelkentText(value=''){
  return String(value||'').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&quot;/gi,'"').replace(/&#39;|&apos;/gi,"'").replace(/&lt;/gi,'<').replace(/&gt;/gi,'>');
}
function parseSelkentClubGroundOptions(html=''){
  const text=decodeSelkentText(String(html||'').replace(/<script\b[\s\S]*?<\/script>/gi,' ').replace(/<style\b[\s\S]*?<\/style>/gi,' ').replace(/<br\s*\/?\s*>/gi,'\n').replace(/<\/[^>]+>/g,'\n').replace(/<[^>]+>/g,' '));
  const lines=text.split(/\n+/).map(x=>x.replace(/\s+/g,' ').trim()).filter(Boolean),lower=lines.map(x=>x.toLowerCase());
  const home=lower.indexOf('home ground'),other=lower.indexOf('other grounds'),colours=lower.indexOf('club colours');
  if(home<0)return[];
  const out=[],push=(name,address)=>{name=String(name||'').trim();address=String(address||'').trim();if(!name||!address)return;const key=(name+'|'+address).toLowerCase();if(!out.some(x=>(x.name+'|'+x.address).toLowerCase()===key))out.push({name,address});};
  const homeEnd=other>home?other:(colours>home?colours:Math.min(lines.length,home+3));
  if(home+2<lines.length&&home+2<=homeEnd)push(lines[home+1],lines[home+2]);
  if(other>home){const end=colours>other?colours:lines.length;for(let i=other+1;i+1<end;i+=2)push(lines[i],lines[i+1]);}
  return out;
}
async function fetchClubDirectoryDetails(teamName){
  ...
  const cached=state.selkent.directoryDetails[key]||{};
  if(cached.clubName&&Array.isArray(cached.groundOptions)&&cached.groundOptions.length)return cached;
  try{
    const r=await fetch(`${SELKENT_DIRECTORY_API}?team=${encodeURIComponent(teamName)}`,{cache:'no-store'});
    if(!r.ok)throw new Error(`HTTP ${r.status}`);
    const d=await r.json(),homeGround=d.home_ground&&d.home_ground.name&&d.home_ground.address?[{name:String(d.home_ground.name).trim(),address:String(d.home_ground.address).trim()}]:[],otherGrounds=Array.isArray(d.other_grounds)?d.other_grounds.map(g=>({name:String(g?.name||'').trim(),address:String(g?.address||'').trim()})).filter(g=>g.name&&g.address):[],apiGrounds=[...homeGround,...otherGrounds];
    const sourceUrl=d.source_url||cached.sourceUrl||'',idMatch=String(sourceUrl).match(/\/public\/clubs\/(\d+)/i),clubId=d.club_id||cached.clubId||(idMatch?Number(idMatch[1]):null);
    const detail={...cached,clubId,clubName:d.club_name||cached.clubName||teamName,colours:shirtColoursOnly(d.club_colours||cached.colours||'TBC'),groundName:d.home_ground?.name||cached.groundName||'TBC',address:d.home_ground?.address||cached.address||'TBC',sourceUrl,groundOptions:apiGrounds,logoUrl:d.logo_url||cached.logoUrl||'',logoVerified:d.logo_verified===true||cached.logoVerified===true};
    const selkentPage=detail.clubId?`https://www.selkent.org.uk/public/clubs/${detail.clubId}`:(/^https:\/\/(?:www\.)?selkent\.org\.uk\/public\/clubs\//i.test(detail.sourceUrl)?detail.sourceUrl:'');
    if(!detail.groundOptions.length&&selkentPage){try{const page=await nativeHttp(selkentPage);if(page.status>=200&&page.status<400){const parsed=parseSelkentClubGroundOptions(page.body);if(parsed.length)detail.groundOptions=parsed;}}catch(_){}}
    if(!detail.groundOptions.length&&knownKit(detail.groundName)&&knownKit(detail.address))detail.groundOptions=[{name:detail.groundName,address:detail.address}];
    ...
    return detail;
  }catch(_){return Object.keys(cached).length?cached:null;}
}
...
async function ensureFixtureKitColours(fixture){
  ...
  if(detail){
    ...
    const grounds=Array.isArray(detail.groundOptions)?detail.groundOptions:[];if(grounds.length===1){fixture.groundName=fixture.groundName||grounds[0].name||'';fixture.address=fixture.address||grounds[0].address||'';}
...
function fixtureOverride(f={}){
  state.selkent=state.selkent||{};state.selkent.fixtureOverrides=state.selkent.fixtureOverrides||{};
...
function renderParentFamilyControls(){
 const wrap=document.getElementById('parent-family-account');if(!wrap)return;
 const parent=CLOUD_MODE&&currentRole==='parent';wrap.classList.toggle('hidden',!parent);if(!parent)return;
 const select=document.getElementById('parent-family-team-select'),teams=window.ClubHubCloud?.visibleTeamList?.()||[],current=window.ClubHubCloud?.currentTeam?.();
 if(select){const keep=select.value||current?.id||'';select.innerHTML=teams.map(t=>`<option value="${esc(t.id)}">${esc(matchTeamLabel([t.ageGroup,t.teamName].filter(Boolean).join(' ')))}</option>`).join('');if([...select.options].some(o=>o.value===String(keep)))select.value=String(keep);}
...
function renderAnnouncements(){
  const panel=document.getElementById('home-club-notices'),list=document.getElementById('home-club-notices-list'),badge=document.getElementById('home-notices-unread'),nextCard=document.getElementById('home-next-match');
  ...
  if(panel)panel.classList.toggle('hidden',!important.length);if(nextCard)nextCard.classList.toggle('hidden',!!important.length);if(badge){badge.textContent=`${unread} unread`;badge.classList.toggle('hidden',!unread);}
...
function valiantsKitIconHtml(colours='TBC'){
  const [primary]=kitColourPair(colours),dark=String(colours).toLowerCase().includes('blue')?'#17365d':'#0f6f3b';
  return `<svg class="match-team-kit valiants-kit" viewBox="0 0 72 68" role="img" aria-label="${esc(colours||'Kit colours TBC')}"><path d="M22 8 31 4h10l9 4 15 9-8 14-8-5v36H23V26l-8 5-8-14z" fill="${primary}" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><g fill="${dark}" opacity=".28"><path d="M25 20l7 4-6 5zM42 14l7 5-8 4zM33 34l8 4-7 6zM46 48l6 4-7 5zM25 50l7 3-6 6z"/></g><g fill="none" stroke="#fff" stroke-width="2"><path d="M20 10l11-5M17 13l14-7M52 10L41 5M55 13L41 6"/></g><path d="M31 4c1 5 9 5 10 0" fill="none" stroke="currentColor" stroke-width="2"/></svg>`;
}
function teamKitIconHtml(teamName='',colours='TBC'){return pilotKitDefaults(teamName).home?valiantsKitIconHtml(colours):kitIconHtml(colours);}

function shirtColoursOnly(text=''){
  const raw=String(text||'').replace(/\s+/g,' ').trim();if(!raw)return'TBC';
  const first=raw.split(/[;,]/)[0].trim();
  const clean=first.replace(/(?:shirts?|jerseys?|tops?)/ig,'').replace(/\s{2,}/g,' ').replace(/[\s:-]+$/,'').trim();
  return clean||first||'TBC';
}
...
function kitProfileForTeam(teamName='',fallbackHome=''){
  ...
  const key=selkentNorm(teamName),stored=state.selkent.kitProfiles[key]||{},detail=state.selkent?.directoryDetails?.[key]||{},pilot=pilotKitDefaults(teamName);
  const ownConfigured=isOwnTeamName(teamName)?(clubSettings().kit_colours||clubSettings().config?.kit_colours||''):'';
  const home=shirtColoursOnly(stored.home||pilot.home||fallbackHome||state.selkent?.kitColours?.[key]||ownConfigured||detail.colours||'TBC');
  const away=shirtColoursOnly(stored.away||pilot.away||'TBC');
  return {key,home,away,homeOverride:stored.home||'',awayOverride:stored.away||''};
}
...
function fixtureKitSelectionKey(f={}){return `${String(f.date||'')}|${fixtureStableKey(f)}`;}
function fixtureKitChoice(f={}){
  state.selkent=state.selkent||{};state.selkent.fixtureKitSelections=state.selkent.fixtureKitSelections||{};
  return state.selkent.fixtureKitSelections[fixtureKitSelectionKey(f)]==='away'?'away':'home';
}
function kitToggleHtml(f={},ctx={}){
  if(!(isCoach()||isAdmin()))return'';
  const key=fixtureKitSelectionKey(f),choice=fixtureKitChoice(f),awayKnown=knownKit(ctx.ownProfile?.away);
  return `<div class="fixture-kit-toggle"><span>Our shirt</span><div role="group" aria-label="Choose our shirt for this fixture"><button type="button" class="${choice==='home'?'active':''}" data-fixture-kit-select="${esc(key)}" data-kit-choice="home">Home</button><button type="button" class="${choice==='away'?'active':''}" data-fixture-kit-select="${esc(key)}" data-kit-choice="away" ${awayKnown?'':'disabled'}>Away${awayKnown?'':' · not set'}</button></div></div>`;
}
function setFixtureKitChoice(key,choice){
  if(!(isCoach()||isAdmin()))return;
  const all=[...upcomingFixtures(),...(state.matches||[])],f=all.find(x=>fixtureKitSelectionKey(x)===key);if(!f)return toast('Fixture is no longer available');
  const ctx=fixtureOverviewContext(f);if(choice==='away'&&!knownKit(ctx.ownProfile?.away))return toast('Add the away shirt colours first');
  state.selkent=state.selkent||{};state.selkent.fixtureKitSelections=state.selkent.fixtureKitSelections||{};
  const before=state.selkent.fixtureKitSelections[key]||'home';state.selkent.fixtureKitSelections[key]=choice==='away'?'away':'home';
  saveState();auditEvent('fixture_kit_changed','fixture',key,`Selected ${choice==='away'?'away':'home'} shirt for ${f.opponent||'fixture'}`,before,state.selkent.fixtureKitSelections[key]);
  renderNextMatch();renderMatchPageNextFixture();renderSelkentFixtures();renderMatchdayDashboard();
...
function matchOverviewContext(m={}){
  const fixture=linkedFixtureForMatch(m)||{},fixtureLike=Object.keys(fixture).length?fixture:m;
  ...
  const ownProfile=kitProfileForTeam(ownTeam,ownProviderKit),oppProfile=kitProfileForTeam(opponent,oppProviderKit),choice=fixtureKitChoice(fixtureLike);
  const ownKit=choice==='away'&&knownKit(ownProfile.away)?ownProfile.away:ownProfile.home,oppKit=oppProfile.home;
  const away=String(m.venue||'').toUpperCase()==='A',homeTeam=away?opponent:ownTeam,awayTeam=away?ownTeam:opponent;
  const homeKit=away?oppKit:ownKit,awayKit=away?ownKit:oppKit;
  const homeKitType=isOwnTeamName(homeTeam)?(choice==='away'?'Away shirt':'Home shirt'):'Home shirt',awayKitType=isOwnTeamName(awayTeam)?(choice==='away'?'Away shirt':'Home shirt'):'Home shirt';
  const clash=kitsClash(homeKit,awayKit);let kitWarning='';
  if(clash){
    if(choice==='home'&&knownKit(ownProfile.away)&&!kitsClash(ownProfile.away,oppKit))kitWarning=`⚠️ Kit clash likely. Switch ${ownTeam} to the away shirt before kick-off.`;
    else if(choice==='away')kitWarning='⚠️ The selected away shirt still appears to clash. Agree an alternative before kick-off.';
    else if(!knownKit(ownProfile.away))kitWarning=`⚠️ Kit clash likely. ${ownTeam} away shirt is not recorded yet.`;
    else kitWarning='⚠️ Kit clash likely. Confirm shirt colours with the opposition before kick-off.';
  }
  const venueDetail=away?oppDetail:ownDetail,override=fixtureOverride(fixtureLike),grounds=Array.isArray(venueDetail.groundOptions)?venueDetail.groundOptions:[],one=grounds.length===1?grounds[0]:null;
  let candidateGround=m.groundName||fixture.groundName||'',candidateAddress=m.address||fixture.address||'';if(!override.confirmedAt&&grounds.length>1&&selkentNorm(candidateGround)===selkentNorm(venueDetail.groundName||'')){candidateGround='';candidateAddress='';}
  const ground=override.confirmedAt?(override.groundName||'Ground TBC'):(candidateGround||one?.name||'Ground TBC'),address=override.confirmedAt?(override.address||'Address TBC'):(candidateAddress||one?.address||'Address TBC');
...
function renderFixtureOverview(prefix,f){
  ...
  const toggle=document.getElementById(`${prefix}-kit-toggle`);if(toggle){toggle.innerHTML=kitToggleHtml(f,ctx);toggle.classList.toggle('hidden',!toggle.innerHTML);}
...
function renderMatchOverview(m){
  ...
  if(map){map.classList.toggle('hidden',!ctx.mapHref);if(ctx.mapHref)map.href=ctx.mapHref;else map.removeAttribute('href');}
  if(warning){warning.innerHTML=kitWarningHtml(ctx);warning.classList.toggle('hidden',!warning.innerHTML);}
  const toggle=document.getElementById('match-detail-kit-toggle');if(toggle){toggle.innerHTML=kitToggleHtml(ctx.fixtureLike,ctx);toggle.classList.toggle('hidden',!toggle.innerHTML);}
...
function renderClubDetails(teamName=''){
  ...
  if(admin){admin.classList.toggle('hidden',!(isAdmin()||isCoach()));const home=document.getElementById('club-kit-home-override'),away=document.getElementById('club-kit-away');if(home)home.value=shirtColoursOnly(ctx.profile.homeOverride||ctx.profile.home);if(away)away.value=knownKit(ctx.profile.away)?shirtColoursOnly(ctx.profile.away):'';}
}
...
function saveClubKitProfile(){
  if(!(isAdmin()||isCoach()))return toast('Club staff access is required');
  ...
  const home=shirtColoursOnly((document.getElementById('club-kit-home-override')?.value||'').trim()),away=shirtColoursOnly((document.getElementById('club-kit-away')?.value||'').trim());
  const next={};if(knownKit(home))next.home=home;if(knownKit(away))next.away=away;
...
async function refreshTeamMembers(quiet=false){
 ...
 try{
   ...
   const rowHtml=r=>{
     const staff=['club_admin','coach','assistant_coach'].includes(r.role),parent=r.role==='parent',player=r.role==='player';
     const roleText=r.role==='club_admin'?'Club Admin · Coach':r.role==='assistant_coach'?'Assistant Coach':r.role==='coach'?'Coach':player?'Player profile':'Parent';
     const pill=r.role==='club_admin'?'Admin Coach':r.role==='assistant_coach'?'Assistant':r.role==='coach'?'Coach':player?'Player':'Parent';
...
function openParentRequestReview(requestId){
 ...
 const req=__pendingParentRequests.find(r=>String(r.request_id)===String(requestId));
 const dlg=document.getElementById('parent-request-review-dialog'),list=document.getElementById('parent-request-player-list');
 if(!req||!dlg||!list)return toast('Parent request is no longer available.');
 document.getElementById('parent-request-review-id').value=String(req.request_id||'');
 document.getElementById('parent-request-review-parent').textContent=req.parent_name||'Parent';
 document.getElementById('parent-request-review-child').textContent=req.child_name||'Not supplied';
 document.getElementById('parent-request-review-email').textContent=req.parent_email||'Not supplied';
 list.innerHTML=activePlayers().map(p=>`<label class="parent-link-player parent-request-player"><input type="radio" name="parent-request-player" value="${esc(p.name)}"/><span>${miniJerseyHTML(p.number,p.role,'compact')}</span><span>${esc(p.name)}</span></label>`).join('')||'<div class="empty-state compact-empty">Add an active player to the squad before approving this request.</div>';
 dlg.showModal();
}
async function submitParentRequestReview(e){
 e.preventDefault();
 if(!['admin','coach','assistant_coach'].includes(currentRole))return toast('Staff access is required to review parent requests.');
 const requestId=document.getElementById('parent-request-review-id')?.value||'';
 const selected=document.querySelector('#parent-request-player-list input[name="parent-request-player"]:checked')?.value||'';
 if(!selected)return alert('Select the correct active squad player before approving this request.');
 const supplied=document.getElementById('parent-request-review-child')?.textContent||'the submitted child';
 if(!confirm(`Approve this request and link "${supplied}" to ${selected}? Existing sibling and other-team links will be kept.`))return;
 try{
   await window.ClubHubCloud.approveParentRequest(requestId,selected);
   auditEvent('parent_request_approved','access',requestId,`Approved parent request and linked ${selected}`);
   document.getElementById('parent-request-review-dialog')?.close();toast(`Parent access approved for ${selected}`);
   __teamMembersStamp=0;await refreshTeamMembers(false);await refreshNotifications(false);
 }catch(err){alert(err.message||err);}
}
async function openParentLinkDialog(userId,parentName='Parent'){
 ...
 document.getElementById('parent-link-user-id').value=userId;document.getElementById('parent-link-title').textContent=`Edit player links · ${parentName}`;
...
async function saveParentLinkDialog(e){
 ...
 try{await window.ClubHubCloud.saveParentPlayerLinks(userId,checked);auditEvent('player_link_updated','access',userId,checked.length?`Updated this team’s parent links: ${checked.map(x=>x.name).join(', ')}`:'Cleared this team’s parent player links',null,checked);document.getElementById('parent-link-dialog')?.close();toast(checked.length?'Player links saved':'Player links cleared');__teamMembersStamp=0;await refreshTeamMembers(false);}catch(err){alert(err.message||err);}
}
...
document.addEventListener('click',e=>{
  ...
  const parentRequest=e.target.closest('[data-review-parent-request-id]');if(parentRequest){openParentRequestReview(parentRequest.dataset.reviewParentRequestId);return;}
...
document.getElementById('parent-request-review-form')?.addEventListener('submit',submitParentRequestReview);
