...
check('versionCode 2226','versionCode 2226' in gradle)
check('versionName 2.2.26',"versionName '2.2.26'" in gradle)
check('applicationId preserved',"applicationId 'com.grassrootsclubhub.universal'" in gradle)
check('native marker version','GrassrootsClubHub/2.2.26' in main)
...
check('About version 2.2.26','App version 2.2.26' in index)
check('manifest name PitchKind',manifest.get('name')=='PitchKind' and manifest.get('short_name')=='PitchKind')
check('manifest version 2.2.26',manifest.get('version')=='2.2.26')
...
check(
    'v2.2.26 native User-Agent constant',
    'private static final String NATIVE_USER_AGENT' in main
    and 'GrassrootsClubHub/2.2.26' in main
...
check(
    'v2.2.26 workflow and artifact gate',
    'Android APK v2.2.26 PitchKind' in apk_workflow
    and 'verify_v2_2_26_pitchkind_ui.py' in apk_workflow
    and 'App version 2.2.26' in apk_workflow
    and 'PitchKind-v2.2.26-debug.apk' in apk_workflow
    and 'GrassrootsClubHub/2.2.26' in apk_workflow,
)
...
check('confirmation and share actions are explicit and visible to staff',
    ...
    and 'Generate match info image' in index
    and 'id="next-match-share"' in index
    and 'id="next-match-share" data-requires-edit' not in index
    and 'share.disabled=!confirmed' in app)
check('match image requires confirmed fixture and uses badge-first identity',
    'Confirm fixture details before generating the match image' in app
