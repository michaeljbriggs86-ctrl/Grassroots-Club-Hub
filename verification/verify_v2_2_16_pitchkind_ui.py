...
check('versionCode 2216','versionCode 2216' in gradle)
check('versionName 2.2.16',"versionName '2.2.16'" in gradle)
check('applicationId preserved',"applicationId 'com.grassrootsclubhub.universal'" in gradle)
check('native marker version','GrassrootsClubHub/2.2.16' in main)
...
check('About version 2.2.16','App version 2.2.16' in index)
check('manifest name PitchKind',manifest.get('name')=='PitchKind' and manifest.get('short_name')=='PitchKind')
check('manifest version 2.2.16',manifest.get('version')=='2.2.16')
...
check(
    'v2.2.16 native User-Agent constant',
    'private static final String NATIVE_USER_AGENT' in main
    and 'GrassrootsClubHub/2.2.16' in main
...
check(
    'v2.2.16 workflow and artifact gate',
    'Android APK v2.2.16 PitchKind' in apk_workflow
    and 'verify_v2_2_16_pitchkind_ui.py' in apk_workflow
    and 'App version 2.2.16' in apk_workflow
    and 'PitchKind-v2.2.16-debug.apk' in apk_workflow
    and 'GrassrootsClubHub/2.2.16' in apk_workflow,
)
...
check(
    'coach can correct match-specific kit colours',
    'id="match-kit-correction"' in index
    and 'id="match-home-kit-colours"' in index
    and 'id="match-away-kit-colours"' in index
    and 'm.homeKitColours=home' in app
    and 'm.awayKitColours=away' in app,
)
check(
...
check(
    'team kit profiles persist home override and away kit knowledge',
    'kitProfiles: {}' in app
    and 'kitProfiles: data.selkent?.kitProfiles' in app
    and "function kitProfileForTeam(teamName='',fallbackHome='')" in app
    and "away:stored.away||'TBC'" in app,
)
check(
    'kit clash warning and alternate kit selection are wired',
    'function kitsClash' in app
    and '⚠️ Kit clash likely.' in app
    and 'alternateSafe' in app
    and 'id="next-match-kit-warning"' in index
    and 'id="matches-next-kit-warning"' in index
    and 'id="matchday-dashboard-kit-warning"' in index
    and 'kit-clash-warning' in styles,
)
check(
    'Club Admin can maintain missing away kit colours',
    'id="club-detail-dialog"' in index
    and 'id="club-kit-away"' in index
    and 'id="save-club-kit-profile"' in index
    and 'function saveClubKitProfile()' in app
    and "if(!isAdmin())return toast('Club Admin access is required')" in app,
)
check(
